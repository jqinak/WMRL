...
def get_img_preprocessor(source: str, target: str, img_size: int = 224):
    ...
    resize = dt.transforms.WrapTorchTransform(
        lambda x: v2f.resize(x, [img_size, img_size], antialias=True),
        source=source,
        target=target,
    )
...
def get_column_normalizer(dataset, source: str, target: str):
    ...
    def norm_fn(x):
        ...
        x = x.float()
        local_mean = mean.to(dtype=x.dtype, device=x.device)
        local_std = std.to(dtype=x.dtype, device=x.device)
        return ((x - local_mean) / local_std).float()
