...
run_id=${RUN_ID:-qwenPI_rl_test_terminal}
...
: "${WMRL_TRAJ_REWARD_SPARSE_CHUNK_END:=false}"
: "${WMRL_TRAJ_REWARD_DENSE_MICRO:=false}"
: "${WMRL_TRAJ_REWARD_TERMINAL:=true}"
# wandb 必须用 trainer.wandb.*（根上 wandb_entity 会触发 struct 报错）
: "${WMRL_WANDB_ENTITY:=jqinak-hkust}"
: "${WMRL_WANDB_PROJECT:=${run_id}}"

python3 -u -m main_wmrl_qwenpi \
  trainer.total_training_steps=1000 \
  trainer.save_interval=10 \
  trainer.output_dir=/project/peilab/qjl/2026/wmrl/playground/Checkpoints/${run_id} \
