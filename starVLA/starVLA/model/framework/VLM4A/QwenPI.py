...
class Qwen_PI(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # LayerwiseFM builds len(transformer_blocks) == num_vl_layers; each block needs one
        # element in vl_embs_list from the VLM (last N entries of output_hidden_states).
        # The backbone's layer count (e.g. 24 for Qwen3.5-VL-4B) must match — hardcoding 36
        # causes IndexError when fewer hidden states exist.
        llm = self.qwen_vl_interface.model
        mc = llm.config
        num_vl_layers = getattr(mc, "num_hidden_layers", None)
        if num_vl_layers is None and getattr(mc, "text_config", None) is not None:
            num_vl_layers = getattr(mc.text_config, "num_hidden_layers", None)
        if num_vl_layers is None and hasattr(llm, "model") and hasattr(llm.model, "language_model"):
            num_vl_layers = len(llm.model.language_model.layers)
        if num_vl_layers is None:
            num_vl_layers = 36
            logger.warning(
                "Could not read num_hidden_layers from VLM; defaulting to 36. "
                "If Layerwise forward raises IndexError, set framework.qwenvl.num_vl_layers in YAML to your backbone depth."
            )
        llm_hidden_size = mc.hidden_size
        self.config.framework.qwenvl.vl_hidden_dim = llm_hidden_size
        self.config.framework.qwenvl.num_vl_layers = int(num_vl_layers)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="/project/peilab/qjl/2026/starVLA/examples/LIBERO/train_files/qwen35vlPI_libero.yaml",
